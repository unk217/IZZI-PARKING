        ...
        <div v-for="(item, index) in menu" :key="index">
          <v-list-item v-if="!item.sub" link :to="item.to" exact>
            <v-list-item-icon class="mr-3 my-3">
              <v-icon size="23" :color="item.icon_color" class="botone">{{
                item.icon
              }}</v-icon>
            </v-list-item-icon>
          ...
          <v-list-group v-else>
            ...
            <v-list-item
              link
              v-for="(el, index) in item.sub"
              :key="'sub-' + index"
              exact
              :to="el.to"
            >
              <v-list-item-icon class="mr-3 my-3">
                <v-icon size="23" :color="el.icon_color" class="botone">{{
                  el.icon
                }}</v-icon>
              </v-list-item-icon>
              <v-list-item-title class="text-title">
...
export default {
  ...
  methods: {
    // selectSub(item) {
    //   this.menu.map((e) => {
    //     if (e.sub && item.text == e.text) e.state_sub_menu = !e.state_sub_menu;
    //     else e.state_sub_menu = false;
    //   });
    // },
  },
  created() {
